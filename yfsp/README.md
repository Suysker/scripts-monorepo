...
- **点击画面播放/暂停**：在播放画面区域单击即可切换播放状态（双击仍保留给播放器自身行为，例如全屏）。
- **RTX VSR 兼容优化**：桌面端隐藏播放器透明覆盖层，并在需要时把全屏切到 `<video>` 原生全屏（最大限度减少覆盖层，提升 VSR 触发概率；代价是站点自定义控制层/弹幕/字幕在原生全屏里不可用，退出全屏请按 `Esc`）。
...
- NVIDIA RTX VSR 的启用取决于显卡驱动/浏览器/硬件加速策略；若视频走 DRM 保护通路或浏览器未启用硬件加速，脚本无法强制开启。
